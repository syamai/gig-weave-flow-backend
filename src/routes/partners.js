...
const getPartner = asyncHandler(async (req, res) => {
  ...
  const partner = await supabasepartnerProfile.findUnique({
    where: { id },
    include: {
      profile: true,
      partnerTechStacks: {
...
const getMyPartnerProfile = asyncHandler(async (req, res) => {
  ...
  const partnerProfile = await supabasepartnerProfile.findUnique({
    where: { userId },
    include: {
      profile: true,
      partnerTechStacks: {
...
const createPortfolio = asyncHandler(async (req, res) => {
  ...
  const portfolio = await supabaseportfolio.create({
    data: {
      partnerId: partnerProfile.id,
      title,
      description,
...
const updatePortfolio = asyncHandler(async (req, res) => {
  ...
  // 포트폴리오 소유자 확인
  const portfolio = await supabaseportfolio.findFirst({
    where: {
      id,
  ...
  const updatedPortfolio = await supabaseportfolio.update({
    where: { id },
    data: updateData
  });
...
const deletePortfolio = asyncHandler(async (req, res) => {
  ...
  await supabaseportfolio.delete({
    where: { id }
  });
